...
def check_unsafe(lista: list):
    """
    Retorna True se for unsafe. False se for safe.
    """
    dec = False
    cre = False
    ...
    if abs(lista[1] - lista[0]) >= 4:
        return True

    for i in range(2, len(lista)):
        # Verifica as condições de "unsafe"
        if (lista[i] > lista[i-1] and dec) or \
           (lista[i] < lista[i-1] and cre) or \
           (i + 1 < len(lista) and lista[i] == lista[i+1]) or \
           lista[i] == lista[i-1] or \
           (i + 1 < len(lista) and abs(lista[i] - lista[i+1]) > 3) or \
           abs(lista[i] - lista[i-1]) > 3:
            return True

    return False

# Testes
lista_safe = [1, 2, 3, 4, 5]
lista_unsafe = [1, 3, 2]

print(check_unsafe(lista_safe))  # Esperado: False (safe)
print(check_unsafe(lista_unsafe))  # Esperado: True (unsafe)
print(check_unsafe([1, 2, 3, 4, 5, 4]))  # Esperado: True (unsafe)
print(check_unsafe([1, 2, 7]))  # Esperado: True (unsafe)
print(check_unsafe([9, 7, 6, 2, 1]))  # Esperado: True (unsafe)
print(check_unsafe([8, 6, 4, 4, 1]))  # Esperado: True (unsafe)
print(check_unsafe([1, 3, 6, 7, 9]))  # Esperado: False (safe)
print(check_unsafe([7, 6, 4, 2, 1]))
print(check_unsafe([1, 2, 7, 8, 9]))
print(check_unsafe([1, 3, 2, 4, 5]))


print(input[300])
print(check_unsafe(input[300]))

safes = 0

for i in range(len(input)):
    if check_unsafe(input[i]) == False:
        print(input[i])
        safes+=1

lista_teste = [83, 87, 89, 92, 93, 95, 97]
print(check_unsafe(lista_teste))


print(f"Resposta final: {safes} relatórios seguros.")
